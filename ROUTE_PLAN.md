...
The type-D parity sublayer `D~I_n(k,k) = B~I_n(k,k) ∩ D_n` is a proved rank result on the same two standard channels and diagonal cells (replay in `type_d/`). The H_m odd residual is closed by the S8 certificate.
Route: core theorem replay, type-D sublayer replay (`type_d/`), S12 separator replay, optional finite n=8 appendix replay, human PDF review, then final public-repo freeze.
